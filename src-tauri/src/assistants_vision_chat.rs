...
#[tauri::command]
pub async fn assistants_vision_chat_test(awindow: Window,
    ...
    match assistants_vision_chat_example(postData.message.unwrap_or_default().as_str()).await.map_err(|e| format!("{:?}", e)) {
        Ok(_) => {
            Ok("テスト終了".to_string())
        }
        Err(err) => {
